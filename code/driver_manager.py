...
def visit_site(db, process, driver, domain, url, temp_folder, cache, update_ublock, geo_db):
    ...
    if BROWSER_SCRIPTS_ENABLED:
        try:
            domain.values["frontend_load_time"] = script_values["frontend_load_time"]
            domain.values["backend_load_time"] = script_values["backend_load_time"]
            domain.values["doc_height"] = script_values["doc_height"]
